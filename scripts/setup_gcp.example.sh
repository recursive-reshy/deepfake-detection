...
# =============================================================================
# STEP 2b — BUDGET ALERT
# =============================================================================
step "2b. Creating budget alert"

timeout 30 gcloud billing budgets create \
  --billing-account="${BILLING_ACCOUNT_ID}" \
  --display-name="Deepfake Detection Budget" \
  --budget-amount=40SGD \
  --threshold-rule=percent=0.5 \
  --threshold-rule=percent=0.9 2>/dev/null || \
  warn "Budget alert may already exist or billing budgets API not enabled — check GCP Console to confirm"

success "Budget alert created — email notification at 50% and 90% of 40SGD"
...
if gcloud secrets describe "${SECRET_NAME}" \
  ...
  [[ -z "${SENDGRID_API_KEY}" ]] && error "SENDGRID_API_KEY is not set. Add it to the configuration block."

  echo -n "${SENDGRID_API_KEY}" | gcloud secrets create "${SECRET_NAME}" \
...
timeout 30 gcloud alpha monitoring policies create \
  --policy-from-file=/tmp/vertexai_alert.json \
  --project="${PROJECT_ID}" 2>/dev/null || \
  warn "Vertex AI alert policy may already exist or requires alpha components — set it up manually in Cloud Console if needed"
